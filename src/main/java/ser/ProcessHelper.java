...
public class ProcessHelper {
    ...
    public IProcessInstance buildNewProcessInstanceForName(String name){
        try{
            log.info("Building new Process Instance with Name: " + name );
            if(name ==null) return null;
            IBpmService bpm = session.getBpmService();
            IProcessType processType = bpm.getProcessTypeByName(name);
            if(processType ==null) {
                log.error("Process Type with ID couldn't be found");
                return null;
            }
